...
try {
  ...
  console.log(
    `Loaded ${arBook.length} Arabic entries and ${enBook.length} English entries`
  );
} catch (error) {
      ...
      children: Array.isArray(arItem.children)
        ? arItem.children
            ...
            .map((arChild) => {
              ...
              if (!enChildItem) {
                console.log(
                  `Warning: No English translation found for child ID ${arChild.ID} in parent ${arItem.ID}`
                );
                return arChild;
              }
              ...
              return {
                ...
                LANGUAGE_ARABIC_TRANSLATED_TEXT:
                  enChildItem.LANGUAGE_ARABIC_TRANSLATED_TEXT,
              };
            })
